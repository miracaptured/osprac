 ...
 * В цикле родительского процесса после записи сообщения
 * счетчик уменьшается с помощью операции D, семафор блокируется,
 * дочерний процесс читает сообщение.
 * После прочтения и записи сообщения счетчик увеличивается с помощью операции A,
 * блокировка с семафора снимается, родительский процесс может прочесть сообщение.
 */
...
int main() {
    ...
    if ((semid = semget(key, 1, 0666)) < 0) {
        // Если доступа нет, пытаемся создать
    ...
    for (i = 0; i < N; ++i) {
        ...
        if (result < 0) {
            ...
            // Проверяем, что сообщение записалось целиком и полностью
            if (size != 16) {
            ...
            // Уменьшаем счетчик
            // Так блокируем процесс родителя, пока работает ребенок
            ...
            // Печатаем прочитанное сообщение
